...
/*       我们还需要为button建立一个监听器，我们需要将编辑框的内容发送到 RecyclerView 上：
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(!content.equals("")) {
                    msgList.add(new MsgChatDetailsInfoType(content,MsgChatDetailsInfoType.TYPE_SEND));
...
//                自定义一问一答
                if(msgList.size() == 2){
                    msgList.add(new MsgChatDetailsInfoType("What's your name?",MsgChatDetailsInfoType.TYPE_RECEIVED));
                    adapter.notifyItemInserted(msgList.size()-1);
                    msgRecyclerView.scrollToPosition(msgList.size()-1);
                }
                if(msgList.size() == 4){
                    msgList.add(new MsgChatDetailsInfoType("Nice to meet you,Bye!",MsgChatDetailsInfoType.TYPE_RECEIVED));
                    adapter.notifyItemInserted(msgList.size()-1);
                    msgRecyclerView.scrollToPosition(msgList.size()-1);
                }
            }
    ...
    private List<MsgChatDetailsInfoType> getData(){
        ...
        list.add(new MsgChatDetailsInfoType("Hello",MsgChatDetailsInfoType.TYPE_RECEIVED));
        return list;
    }
}
